...
func findSymmetryFromStart(rc []string) int {
	...
	index := 1
	for {
		i := tolerantIndex(rc[index:], start)
		if i == -1 {
			break
		}
		if isPalindromeExceptForOneCharacter(rc[:i+index+1]) {
...
func tolerantIndex(slc []string, s string) int {
	for i := range slc {
		if countWrongChars(slc[i], s) <= 1 {
			return i
		}
	}
	return -1
...
func isPalindromeExceptForOneCharacter(s []string) bool {
	if len(s)%2 != 0 {
	...
	for i := 0; i < len(s)/2; i++ {
		left := s[i]
		right := s[len(s)-i-1]
		wrongChars += countWrongChars(left, right)
		if wrongChars > 1 {
			return false
		}
	}
...
func countWrongChars(left, right string) int {
	wrong := 0
